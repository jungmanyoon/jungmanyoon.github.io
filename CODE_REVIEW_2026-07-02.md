...
- 2026-07-03(6): 남은 low/info 직접+에이전트 처리 시작. misc: localStorage quota 방어, e2e 반응형 의미화 반영. 구조/위험 5건 보류(사유 하단). stores 7건 에이전트 완료(typecheck0/test164). calc·components 병렬 진행중.
...
- [x] **(핵심계산)** 제법변환에서 저수화율 레시피 시 액체가 중종/폴리시에도 본반죽에도 없이 소실 (질량 비보존) — `src/utils/calculations/methodConversion.js:89`
...
- [x] **(레시피/변환/공통)** prefermentTemp 0°C가 '사전반죽 없음'으로 오판되어 잘못된 공식 사용 — `src/components/conversion/DDTCalculator.tsx:197`
...
- [x] **(스토어)** selectFilteredRecipes 정렬이 name/category 널가드 없이 localeCompare 호출 — `src/stores/useRecipeStore.ts:254` (원high)
...
- [x] **(빌드/설정)** base:'/' 고정과 gh-pages 배포/저장소 URL 정합성 위험 — `vite.config.js:7`  (확인: user.github.io 루트 배포라 '/' 정상, 변경 불필요)
...
- [x] **(핵심계산)** 영양표 총중량을 0.4kcal/g 가정으로 역산해 servingsPerContainer가 약 6~7배 과대 — `src/utils/calculations/nutritionCalculator.js:179` (원medium)
- [x] **(핵심계산)** realtimeCalculator 폴리시 변환이 밀가루 없는 물+이스트로 모델링되어 폴리시 정의와 상충 — `src/utils/calculations/realtimeCalculator.ts:258` (원medium)
- [x] **(핵심계산)** 제법변환 시 본반죽 물/이스트 음수 미방어 — `src/utils/calculations/realtimeCalculator.ts:176`
- [x] **(핵심계산)** 팬 프리셋 'small' 표시 부피 630ml가 사다리꼴 보정(0.85) 실제 계산값 535.5ml와 불일치 — `src/utils/calculations/realtimeCalculator.ts:358`
- [x] **(핵심계산)** 수화율 계산이 달걀 전체 중량을 100% 액체로 계산해 과대평가 — `src/utils/calculations/bakersPercentage.ts:91`
...
- [x] **(핵심계산)** 영양/원가 계산의 0-나눗셈 미방어로 NaN/Infinity 전파 가능 — `src/utils/calculations/nutritionCalculator.js:188`
- [x] **(대시보드)** saveRecipeData 의존성 배열 누락으로 비연동 모드에서 원래 팬/원제품 비용적이 stale 값으로 저장됨 — `src/components/dashboard/AdvancedDashboard.tsx:1728` (원medium)
...
- [x] **(대시보드)** useWorker: postMessage 성공/타임아웃 시 error(또는 message) 리스너가 제거되지 않아 누적됨 — `src/hooks/useWorker.ts:102` (원medium)
- [x] **(대시보드)** useWorker: 공유 timeoutRef로 인해 동시 postMessage 호출 시 타임아웃 취소가 어긋남 — `src/hooks/useWorker.ts:65`
...
- [x] **(i18n)** getSubstitutionRules 양방향 부분매칭으로 잘못된 대체규칙 반환 — `src/data/substitutionRules.ts:385` (원medium)
- [x] **(i18n)** 굽기 시간 범위 패턴에서 상한 없을 때 '25- min' 잔여 하이픈 출력 — `src/data/processStepTranslations.ts:158` (원medium)
- [x] **(i18n)** 재료명 표기 불일치로 영어 표시/교차조회 실패 및 오역 — `src/data/ingredientTranslations.ts:89` (원medium)
- [ ] **(i18n)** 언어 소스 이중화: LanguageDetector(i18nextLng) vs useLocaleStore 지속 상태 — `src/i18n/index.ts:65`
- [x] **(i18n)** 공정 단계 자유 텍스트의 온도가 화씨 사용자에게도 °C로 고정 표기 — `src/hooks/useLocalization.ts:296`
- [x] **(레시피/변환/공통)** DDT 계산기의 얼음 모드가 한 번 켜지면 이후 계산에 고착 — `src/components/conversion/DDTCalculator.tsx:213` (원medium)
- [x] **(레시피/변환/공통)** IngredientTable: 추가/삭제 가능한 편집 행에 index를 key로 사용 — `src/components/recipe/IngredientTable.jsx:96` (원medium)
- [x] **(레시피/변환/공통)** RecipeView: ingredient.type 필터로 베이커스%/수화율/수율 오산출 — `src/components/recipe/RecipeView.jsx:45` (원medium)
- [x] **(레시피/변환/공통)** RecipeCard: div role=button 내부에 편집/삭제 button 중첩(상호작용 요소 중첩) — `src/components/recipe/RecipeCard.tsx:153`
- [x] **(레시피/변환/공통)** PanSelector: 커스텀 팬 type 변경이 부모로 전파되지 않아 stale 값 사용 — `src/components/conversion/PanSelector.jsx:130`
- [x] **(레시피/변환/공통)** MethodSelector: advantages/disadvantages 배열 존재를 가정한 slice 호출 — `src/components/conversion/MethodSelector.jsx:47`
- [x] **(레시피/변환/공통)** YieldLossCalculator/AutocompleteInput: 매 렌더 새 객체·배열로 useMemo 무력화 — `src/components/common/YieldLossCalculator.tsx:124`
- [x] **(보안)** quota 초과 무시 — `src/utils/storage/localStorage.js:36` (원medium)
- [x] **(설정)** 수율손실 탭: 제품 오버라이드 '제거' 버튼이 동작하지 않고, 변경 없이 저장해도 오버라이드로 오인됨 — `src/components/settings/YieldLossSettingsTab.tsx:392` (원medium)
- [x] **(설정)** 환경 탭: 발효 영향 계산이 기준온도 26을 하드코딩해 제법 탭의 baseTemperature 설정을 무시 — `src/components/settings/EnvironmentSettingsTab.tsx:74`
- [x] **(설정)** 제품 탭: 비용적 입력을 비우거나 0 입력 시 즉시 기본값으로 덮어써져 자유 입력이 어려움 — `src/components/settings/ProductSettingsTab.tsx:322`
- [x] **(설정)** 저장 탭: 폴더에서 설정 로드 시 importSettings 실패해도 성공 토스트 표시 — `src/components/settings/StorageSettingsTab.tsx:130`
- [x] **(설정)** 설정 페이지: 파일 가져오기 후 input 값 미초기화 + reader 에러 미처리 — `src/components/settings/SettingsPage.tsx:187`
- [x] **(스토어)** 어떤 persist 스토어도 localStorage 용량 초과(QuotaExceededError)를 처리하지 않음 — `src/stores/useRecipeStore.ts:153` (원medium)
- [x] **(스토어)** importRecipes 중복 제거가 id 없는 레시피에 대해 무력화되어 중복/undefined-id 항목 유입 — `src/stores/useRecipeStore.ts:82` (원medium)
- [x] **(스토어)** useSettingsStore가 partialize/Date 정규화 없이 기본 팬 60여 개를 통째로 persist하고 import 시 날짜 미정규화 — `src/stores/useSettingsStore.ts:426` (원medium)
...
- [x] **(레시피/변환/공통)** RecipeListPage 초기화 useEffect의 빈 의존성 배열(exhaustive-deps 위반) — `src/components/recipe/RecipeListPage.tsx:56`
- [x] **(설정)** 고급 탭: isCustom 판정이 항상 false인 죽은 분기 — `src/components/settings/AdvancedSettingsTab.tsx:169`
- [x] **(스토어)** 팬 부피 0일 때 스케일 배율 division-by-zero로 Infinity/NaN 재료량 — `src/stores/useDashboardStore.ts:371`
...
- [x] **[Medium]** AdvancedDashboard 전반 as any 27건(타입에 없는 yieldStageSelection 읽기/쓰기 포함) — `src/components/dashboard/AdvancedDashboard.tsx`  (부분: 6건 제거, 표준 Recipe와 상이한 저장/로드 구조 7건은 회귀방지 위해 유지)
  - 수정: 확장 필드를 Recipe(또는 확장 인터페이스)에 정식 추가, as any 제거, 옵셔널 체이닝 사용.
- [x] **[Medium]** panScaling.ts calculatePanVolume의 non-null 단언(!)이 undefined 치수를 은폐(NaN 위험) — `src/utils/calculations/panScaling.ts:359`
...
## 보류 항목 (사유 명시, 추후/사용자판단 필요)
- i18n 언어소스 이중화(LanguageDetector vs useLocaleStore): detection order 변경이 언어전환 동작 회귀 위험. useLocaleStore 연동 구조 리팩터 필요 -> 별도 작업.
- vitest/vite/tsconfig alias 중복: tsconfig paths(TS 컴파일러)와 vite/vitest alias(번들러)는 본질적으로 별개 시스템이라 완전 단일출처화 불가. 부분 통합 이득 낮아 보류.
- PWA 수동(public/sw.js·manifest.json) vs VitePWA 이중화: 어느 SW/manifest가 실효인지 런타임(브라우저 설치/오프라인) 검증 필요. 색(theme_color)은 amber 정합 완료. 구조 통합은 런타임 확인 후 별도.
- declarations.d.ts 와일드카드 any: sampleRecipes.js -> .ts화 등 구조 변경 필요. 별도 작업.
- 루트 스크립트(run.py/analyze_excel*.py/create-cafe-post.js) 저장소 잔존: 사용자 자산이라 임의 삭제 지양. 사용자 확인 후 정리 권장.
